...
export function detectionToTiles(detections: readonly TileDetection[]): DetectionToTilesResult {
  ...
  for (const detection of detections) {
    ...
    const tile = mapDetectionClassToTile(detection.class);
    if (tile === undefined) {
  ...
  if (unknownClasses.length > 0) {
    throw new Error(`未知检测类别：${uniquePreserveOrder(unknownClasses).join("、")}`);
  }

  const orderedTiles = clusterRows(mapped).flatMap((row) =>
    [...row.items]
      .sort((left, right) => left.detection.x - right.detection.x)
      .map((item) => item.tile)
  );
  const trimmed = trimTilesOverFour(orderedTiles);
  const confidence = calculateDetectionConfidence(detections);

  return {
    tiles: trimmed.tiles,
    confidence: trimmed.trimmedCount > 0 ? Math.min(confidence, 0.3) : confidence
  };
}

export function mapDetectionClassToTile(className: string): string | undefined {
...
function trimTilesOverFour(tiles: readonly string[]): { tiles: string[]; trimmedCount: number } {
  const counts = new Map<string, number>();
  const kept: string[] = [];
  let trimmedCount = 0;

  for (const tile of tiles) {
    const key = normalizeTileForCount(tile);
  ...
  return { tiles: kept, trimmedCount };
}

function normalizeTileForCount(tile: string): string {
